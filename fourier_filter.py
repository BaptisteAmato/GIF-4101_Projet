...
def fourier_transform(img):
    """Computes the Discrete Fourier Transform of an MxN numpy array

    :params img: The MxN numpy array

    :returns : The magnitude and the phase of the Discrete Fourier Transform
    """
    return numpy.abs(F), numpy.angle(F), numpy.fft.fftshift(numpy.fft.fft2(img))
...
def make_binary(img, algorithm):
    """Computes the binary image on an array given an algorithm
    ...
    if algorithm not in algorithms:
        ...
        threshold = algorithms[algorithm](img)
        binary = img > threshold
        return binary


def gaussian_blur(img, sigma=10, threshold=5e-5, logical_not=False):
    ...
    '''
    gaussian_blur = filters.gaussian(img, sigma=sigma)
    binary_filter = (gaussian_blur > threshold).astype(int)
    im = gaussian_blur * binary_filter
    im = im / numpy.amax(im)
    if logical_not:
        im[im > 0] = 1
...
if __name__ == "__main__":
    ...
    root = "/home/maewanto/Desktop/Tests"
    keepFormat = "*.tif"
